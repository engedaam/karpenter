...
func main() {
	...
	if clusterName != "" {
		...
		fmt.Println(exec.Command("eksctl", "delete", "cluster", "--name", clusterName, "--timeout", "60m", "--wait").String())
		if out, err := exec.Command("eksctl", "delete", "cluster", "--name", clusterName, "--timeout", "60m", "--wait").Output(); err != nil {
