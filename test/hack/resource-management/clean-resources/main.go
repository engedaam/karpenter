...
const karpenterMetricTableName = "sweeperCleanedResources"
...
func main() {
	...
	for i := range resourceTypes {
		...
		if len(ids) > 0 {
			...
			if err = metricsClient.FireMetric(ctx, karpenterMetricTableName, fmt.Sprintf("%sDeleted", resourceTypes[i].String()), float64(len(cleaned)), cfg.Region); err != nil {
				resourceLogger.Errorf("%v", err)
			}
