...
const karpenterMetricTableName = "resourceCount"
...
func main() {
	...
	for i := range resourceTypes {
		...
		if err = metricsClient.FireMetric(ctx, karpenterMetricTableName, fmt.Sprintf("%s", resourceTypes[i].String()), float64(resourceCount), cfg.Region); err != nil {
			resourceLogger.Errorf("%v", err)
		}
